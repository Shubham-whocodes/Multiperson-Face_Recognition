...
from parameters import FILES_PATH, NUMBER_OF_TIMES_TO_UPSAMPLE, FACE_RECOGNITION_MODEL
...
while True:
    ...
    if ret:
        ...
        #resize frame to 1024x576 to display the video if frame is too big
        if current_frame.shape[1] > 1024 or current_frame.shape[0] > 576:
            current_frame_small = cv2.resize(current_frame, (1024, 576))
        else:
